...
Alpine.data('redisMonitorActiveKeysChart', (config) => ({
    init() {
        ...
        Livewire.on('redis-monitor-active-keys-chart-update', ({ keys }) => {
            if (chart === undefined) {
                return
            }
...
Alpine.data('redisMonitorRemovedKeysChart', (config) => ({
    init() {
        ...
        Livewire.on('redis-monitor-removed-keys-chart-update', ({ keys }) => {
            if (chart === undefined) {
                return
            }
            ...
            chart.data.labels = this.labels(keys[config.connection])
            chart.options.scales.y.max = this.highest(keys[config.connection])
            chart.data.datasets[0].data = keys[config.connection]['expired_keys']
            chart.data.datasets[1].data = keys[config.connection]['evicted_keys']
            chart.update()
        })
    },
    labels(data) {
...
Alpine.data('redisMonitorNetworkUsageChart', (config) => ({
    init() {
        ...
        Livewire.on('redis-monitor-network-usage-chart-update', ({ network }) => {
            if (chart === undefined) {
                return
            }

            if (network[config.connection] === undefined && chart) {
                chart.destroy()
                chart = undefined
                return
            }

            chart.data.labels = this.labels(network[config.connection])
            chart.options.scales.y.max = this.highest(network[config.connection])
            chart.data.datasets[0].data = network[config.connection]['redis_network_usage']
            chart.update()
        })
    },
    labels(data) {
